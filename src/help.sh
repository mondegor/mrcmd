...
function mrcmd_help_GROUP_exec() {
  ...
  echo -e "  ${CC_BLUE}--shared-plugins-dir${CC_END} ${CC_CYAN}DIR${CC_END}  Set shared plugins dir for the project"
...
function mrcmd_help_echo_GROUP_plugin_head() {
  ...
  echo -e "$(mrcmd_plugins_lib_get_plugin_var "${pluginName}" "CAPTION")"
}
